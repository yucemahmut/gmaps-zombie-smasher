...
public class ZombieFactory implements IFactory {

	private static final double SPEED = 5.0;
	private MapInformationUtilities mapInfo;

	public ZombieFactory(MapInformationUtilities mapInfo) {
	...
	public Entity getEntity() {
		...
		entity.setCurrentPosition(getRandomPosition());
		entity.setExist(true);
		entity.setIa(new CAIZombie());
		entity.setMarker(new CMarker(CMarker.ZOMBIE_MARKER));
		entity.setMoveSpeed(new CMoveSpeed(SPEED
				* (mapInfo.getZoomLevelMin() - mapInfo.getZoomLevel() + 2)));
	 ...
	 */
	private CCoordinates getRandomPosition() {
		...
		int topLatitude = mapInfo.getTopLatitude();
		int leftLongitude = mapInfo.getLeftLongitude();
		int rightLongitude = mapInfo.getRightLongitude();
		int botLatitude = mapInfo.getBotLatitude();
